...
public class AliyunOssService {
    ...
    public String uploadToFile(MultipartFile file,String filePath) {
        if (file.isEmpty()) {
            return null;
        }
        // 创建OSSClient实例
        ...
        try {
            // 文件路径和名称，这里以上传时间作为文件名
            String fileKey = filePath+"/" + System.currentTimeMillis() + "_" + file.getOriginalFilename();
            // 上传文件
            ossClient.putObject(bucketName, fileKey, file.getInputStream());

            String imageUrl = "https://" + bucketName + "." + endpoint + "/" + fileKey;
            return imageUrl;
        } catch (IOException e) {
            throw new RuntimeException(e);
        } finally {
